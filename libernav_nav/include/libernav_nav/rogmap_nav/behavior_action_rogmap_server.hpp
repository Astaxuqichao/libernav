#pragma once
// New RecoveryRogMapServer using LiberNavActionBase framework.
...
#include "libernav_rogmap_core/recovery.hpp"
...
#include "libernav_base/behavior_action.h"
#include "libernav_base/behavior_execution.h"
 ...
 *        DummyBehavior action server using the LiberNavActionBase framework.
 */
class RecoveryRogMapServer : public nav2_util::LifecycleNode
{
public:
  using BehaviorMap =
    std::unordered_map<std::string, libernav_rogmap_core::Recovery::Ptr>;
  ...
  explicit RecoveryRogMapServer(
    std::shared_ptr<libernav_rog_map::RogMapROS> global_rog_map_ros,
    std::shared_ptr<libernav_rog_map::RogMapROS> local_rog_map_ros,
...
private:
  ...
  // Plugins
  pluginlib::ClassLoader<libernav_rogmap_core::Recovery> plugin_loader_;
  ...
  // Costmaps
  std::shared_ptr<libernav_rog_map::RogMapROS> global_rog_map_ros_;
  std::shared_ptr<libernav_rog_map::RogMapROS> local_rog_map_ros_;
