...
#include "libernav_nav/rogmap_nav/controller_action_rogmap_server.hpp"
#include "libernav_nav/rogmap_nav/behavior_action_rogmap_server.hpp"
...
private:
  ...
  std::shared_ptr<libernav_rog_map::RogMapROS> rog_map_;
  std::unique_ptr<nav2_util::NodeThread> rog_map_thread_;

  // ========== Navigation Servers ==========
  /// Planner server instances for path planning
  std::shared_ptr<libernav_nav::PlannerRogMapServer> planner_server_;
  ...
  std::shared_ptr<libernav_nav::ControllerRogMapServer> controller_server_;
  ...
  std::shared_ptr<libernav_nav::RecoveryRogMapServer> behavior_server_;
  ...
  std::shared_ptr<libernav_rog_map::RogMapROS> selectCostmap(uint8_t costmap_id);

  static uint8_t occupancyToState(libernav_rog_map::OccupancyState state);

  // ========== Robot Configuration ==========
  /// Shared robot state information (position, velocity, etc.)
  libernav_utility::RobotInformation::Ptr robot_info_;
