...
def launch_setup(context, *args, **kwargs):
    bringup_dir = get_package_share_directory('libernav_bringup')
    bt_dir = get_package_share_directory('libernav_rogmap_bt_navigator')
    ...
    lifecycle_nodes = []
    if selected_navigation_type in ('costmap', 'both'):
        lifecycle_nodes.append('costmap/libernav_nav')
    if selected_navigation_type in ('rogmap', 'both'):
        lifecycle_nodes.append('rogmap/libernav_nav')
    if with_route:
    ...
    if selected_navigation_type in ('costmap', 'both'):
        composable_nodes.append(ComposableNode(
            package='libernav_nav',
            plugin='libernav_nav::CostmapNavNode',
            namespace='costmap',
            name='libernav_nav',
            parameters=[
                {'use_sim_time': use_sim_time},
    ...
    if selected_navigation_type in ('rogmap', 'both'):
        composable_nodes.append(ComposableNode(
            package='libernav_nav',
            plugin='libernav_nav::RogMapNavNode',
            namespace='rogmap',
            name='libernav_nav',
            parameters=[
                {'use_sim_time': use_sim_time},
    ...
    lifecycle_manager = Node(
        ...
        name='lifecycle_manager_libernav',
        output='screen',
        ros_arguments=rosout_disabled,
        arguments=['--ros-args', '--log-level', log_level],
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('use_route_server', default_value='False',
                              description='Whether to launch nav2_route with the libernav stack'),
