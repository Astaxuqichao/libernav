...
`navflex_nav` 是 Navflex 的统一导航执行层。它把 costmap 和 ROGMap 后端放在同一
...
RogMapNavNode  -> navflex_rogmap_core plugins -> RogMap::ConstPtr
```
...
`include/navflex_base` 保存 planner/controller/recovery 的执行状态机和 plugin adapter，
`src/costmap_nav`、`src/rogmap_nav` 只处理后端资源装配。规划和控制插件不负责
action 生命周期，也不直接创建地图线程。

ROGMap 后端的地图由 `navflex_rog_map::RogMapROS` 创建一次，并以只读
...
详细实现位于 `include/navflex_base`、`include/navflex_nav/{costmap_nav,rogmap_nav}`
以及对应的 `src` 目录。
