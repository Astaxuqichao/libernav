...
#include "libernav_nav/rogmap_nav/planner_action_rogmap_server.hpp"
...
PlannerRogMapServer::PlannerRogMapServer(
  std::shared_ptr<libernav_rog_map::RogMapROS> rog_map_ros,
  const libernav_utility::RobotInformation::ConstPtr & robot_info,
  const std::string & parent_namespace,
  const rclcpp::NodeOptions & options)
: nav2_util::LifecycleNode(
    "libernav_planner_server", parent_namespace,
    rclcpp::NodeOptions().arguments(
      {"--ros-args", "-r",
        std::string("libernav_planner_server") + ":" +
        std::string("__node:=") +
        std::string("libernav_planner_server")}
    ).enable_rosout(false)),
  name_action_get_path_("compute_path_to_pose"),
  gp_loader_("libernav_rogmap_core", "libernav_rogmap_core::GlobalPlanner"),
  default_ids_{},
  default_types_{},
 ...
 *
 * **Plugin System:**
 * Uses pluginlib to dynamically load libernav_rogmap_core::GlobalPlanner implementations:
 * - NavfnPlanner (default)
 * - SmacPlanner
 * - ThetaStarPlanner
 * - Other custom planners implementing libernav_rogmap_core::GlobalPlanner interface
 *
 * **Error Handling:**
  ...
  const rclcpp_lifecycle::State & /*state*/)
{
  ...
  for (size_t i = 0; i != planner_ids_.size(); i++) {
    try {
      ...
      libernav_rogmap_core::GlobalPlanner::Ptr planner =
  ...
  action_server_get_path_ptr_ = rclcpp_action::create_server<ActionToPose>(
    ...
    std::bind(
      &libernav_nav::PlannerRogMapServer::handleGoalGetPath, this,
      _1, _2),
    std::bind(
      &libernav_nav::PlannerRogMapServer::cancelActionGetPath, this,
      _1),
    std::bind(
      &libernav_nav::PlannerRogMapServer::callActionGetPath, this,
      _1),
 ...
 * 5. Delegate to LiberNavActionBase for actual execution management
 ...
 * 3. LiberNavActionBase manages the execution thread lifecycle
...
void PlannerRogMapServer::callActionGetPath(ServerGoalHandleGetPathPtr goal_handle)
{
  ...
  libernav_rogmap_core::GlobalPlanner::Ptr planner_ptr = nullptr;
  ...
  libernav_nav::PlannerExecution::Ptr planner_execution =
...
libernav_nav::PlannerExecution::Ptr
PlannerRogMapServer::newPlannerExecution(
  const std::string & plugin_name,
  const libernav_rogmap_core::GlobalPlanner::Ptr & plugin_ptr)
{
  auto node = shared_from_this();
  return std::make_shared<libernav_nav::PlannerExecution>(
