#include "libernav_base/controller_action.h"
...
: LiberNavActionBase(node, name, robot_info)
{
  ...
  ControllerExecution::Ptr execution_ptr)
{
  ...
  // libernav_costmap_nav only supports slot 0 (no concurrency_slot in FollowPath)
    ...
    if (slot_it != concurrency_slots_.end() && slot_it->second.in_use) {
      ...
      if (active_handle && active_handle->is_active()) {
        ...
        if (same_controller && same_goal_tolerance) {
          ...
          // to LiberNavActionBase::start(), calls cancel() on the running execution,
  ...
  if (!update_plan) {
    ...
    LiberNavActionBase::start(goal_handle, execution_ptr);
  }
}
  ...
  ControllerExecution & execution)
{
  ...
  auto build_result = [&](uint32_t fallback_outcome,
      ...
      result->dist_to_goal = static_cast<float>(libernav_utility::distance(robot_pose, goal_pose));
      result->angle_to_goal = static_cast<float>(libernav_utility::angle(robot_pose, goal_pose));
      return result;
    };
  ...
  auto check_stuck = [&](const geometry_msgs::msg::PoseStamped & pose,
      ...
      const double moved = libernav_utility::distance(pose, stuck_window_pose);
  ...
  const std::string & message)
{
  auto feedback = std::make_shared<ActionFollowPath::Feedback>();
  {
    ...
    feedback->distance_to_goal =
      static_cast<float>(libernav_utility::distance(robot_pose_, goal_pose_));
    feedback->angle_to_goal =
      static_cast<float>(libernav_utility::angle(robot_pose_, goal_pose_));
