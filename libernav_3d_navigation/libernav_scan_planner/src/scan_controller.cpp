...
#include "libernav_scan_planner/scan_controller.hpp"
  ...
  libernav_rog_map::RogMap::ConstPtr map)
{
  ...
  declare("trajectory_topic", std::string("libernav_scan_planner/optimized_path"));
  ...
  footprint_.type = libernav_rog_map::footprintTypeFromString(
  ...
  libernav_rog_map::validateFootprint(footprint_);
...
void ScanController::cleanup()
{
  ...
  input_trajectory_ = libernav_rogmap_core::Trajectory3D();
  map_.reset();
  planner_.reset();
...
void ScanController::setTrajectory(const libernav_rogmap_core::Trajectory3D & trajectory)
{
  std::lock_guard<std::mutex> lock(mutex_);
...
PLUGINLIB_EXPORT_CLASS(
  libernav_scan_planner::ScanController,
  libernav_rogmap_core::Controller)
