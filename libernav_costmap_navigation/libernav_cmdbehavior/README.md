# libernav_cmdbehavior

`libernav_cmdbehavior` 是一个 Nav2 recovery behavior 插件包，用于通过运行时命令字符串驱动机器人运动。

## 功能

- 提供一个 `nav2_core::Behavior` 插件：`libernav_cmdbehavior::CmdBehavior`
- 支持通过命令控制机器人执行：
...
插件描述文件：`libernav_cmdbehavior_plugin.xml`

- 插件类型：`nav2_core::Behavior`
- 插件名称：`libernav_cmdbehavior/CmdBehavior`
...
在 Behavior Tree 中通过 `LiberNavRecoveryAction` 或 `DummyBehavior` 发送命令：

```xml
<Action ID="LiberNavRecoveryAction"
...
```bash
colcon build --packages-select libernav_cmdbehavior
```
