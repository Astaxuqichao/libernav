...
void RogMapROS::cloudCallback(sensor_msgs::msg::PointCloud2::ConstSharedPtr message)
{
  ...
  // The message frame is authoritative. The configured frame is only a
  // compatibility fallback for drivers which omit header.frame_id.
  const std::string source_frame =
    message->header.frame_id.empty() ? point_cloud_frame_ : message->header.frame_id;
  if (!point_cloud_frame_.empty() && !message->header.frame_id.empty() &&
    point_cloud_frame_ != message->header.frame_id)
  {
    RCLCPP_WARN_THROTTLE(
      get_logger(), *get_clock(), 5000,
      "ROG map point_cloud_frame [%s] differs from message frame [%s]; using message frame",
      point_cloud_frame_.c_str(), message->header.frame_id.c_str());
  }
  try {
...
RCLCPP_COMPONENTS_REGISTER_NODE(libernav_rog_map::RogMapROS)
