# libernav_autonomous_exploration_bt

This package provides a standalone behavior-tree configuration for autonomous
FAEL-style frontier exploration with LiberNav.

The tree repeatedly:

1. calls `LiberNavGetPathAction` on `/rogmap/compute_path_to_pose` with planner ID
   `FrontierAStar`;
2. lets the ROGMap-based `libernav_frontier_planner/FrontierAStarPlanner`
   select and plan to the best frontier viewpoint;
3. sends the returned path to `LiberNavExePathAction` on `/rogmap/follow_path` with
...
```bash
colcon build --packages-select libernav_autonomous_exploration_bt
```

## Launch

```bash
ros2 launch libernav_autonomous_exploration_bt fael_exploration.launch.py
```

Start LiberNav with `navigation_type:=rogmap` before launching exploration. The
