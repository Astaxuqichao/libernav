...
`libernav_rog_map` 是 LiberNav 唯一的 ROGMap 地图拥有者，将原始 ROG-Map 能力包装为
...
主要参数集中在 `libernav_bringup/params/rogmap_params.yaml`：
...
地图接口实现集中在 `include/libernav_rog_map/rog_map.hpp` 和 `src/rog_map.cpp`，ROS
生命周期包装在 `rog_map_ros.*`。
