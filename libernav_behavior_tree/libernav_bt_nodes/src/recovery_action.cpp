...
#include "libernav_bt_nodes/recovery_action.hpp"
...
{
  // LiberNavRecoveryAction
  {
    BT::NodeBuilder builder =
      [](const std::string & name, const BT::NodeConfiguration & config)
      {
        return std::make_unique<libernav_bt_nodes::RecoveryAction>(
          name, "/costmap/behavior_action", config);
      };
    factory.registerBuilder<libernav_bt_nodes::RecoveryAction>(
      "LiberNavRecoveryAction", builder);
  }

  // LiberNavCancelRecovery
  {
    BT::NodeBuilder builder =
      [](const std::string & name, const BT::NodeConfiguration & config)
      {
        return std::make_unique<libernav_bt_nodes::CancelRecoveryAction>(
          name, "/costmap/behavior_action", config);
      };
    factory.registerBuilder<libernav_bt_nodes::CancelRecoveryAction>(
      "LiberNavCancelRecovery", builder);
  }
}
