...
: LiberNavExecutionBase(name, robot_info, node),
...
bool ControllerExecution::start()
{
  ...
  RCLCPP_DEBUG(
    ...
    name_.c_str());
  bool started = LiberNavExecutionBase::start();
  if (!started) {
    RCLCPP_ERROR(
      node_handle_->get_logger(), "[ControllerExecution] LiberNavExecutionBase::start() failed for %s",
      name_.c_str());
    moving_.store(false);
  }
  return started;
