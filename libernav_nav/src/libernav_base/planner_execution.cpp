...
double sumDistance(_Iter _begin, _Iter _end)
{
  ...
  for (_Iter next = _begin + 1; next != _end; ++_begin, ++next) {
    dist += libernav_utility::distance(*_begin, *next);
  }
...
: LiberNavExecutionBase(name, robot_info, node_handle),
  ...
  double tolerance)
{
  ...
  return LiberNavExecutionBase::start();
}
 ...
 *
 * **Purpose:**
 * Implements LiberNavExecutionBase::run() pure virtual method. This is the core
 ...
 *
 * **Thread Context:** Dedicated execution thread (called from LiberNavExecutionBase::start())
 *
 * **State Machine:**
 * ```
 * INITIALIZED
 ...
 * @note Thread must be started via LiberNavExecutionBase::start()
