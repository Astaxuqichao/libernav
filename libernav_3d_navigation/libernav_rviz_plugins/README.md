# libernav_rviz_plugins

RViz integration for manual LiberNav 3D goals. `Goal3DPanel` is a standard
...
server. `libernav_rogmap_bt_navigator` consumes the topic and owns TF conversion,
...
```bash
colcon build --packages-select libernav_rviz_plugins --symlink-install
