# libernav_rogmap_core

ROGMap 后端的插件契约层。它刻意不拥有地图数据、不实现 ROS 传感器回调，也不复制
占据网格；所有插件通过 `libernav_rog_map::RogMap::ConstPtr` 读取同一张地图。

## 数据流

```text
PCD / PointCloud2 / TF
        -> libernav_rog_map::RogMapROS
