...
#include "libernav_scan_planner/single_scan_planner.hpp"

namespace
{

libernav_rog_map::RogMapConfig mapConfig()
{
  libernav_rog_map::RogMapConfig config;
...
{
  libernav_rog_map::Footprint3D result;
  result.type = libernav_rog_map::FootprintType::SPHERE;
  result.radius = 0.12;
  return result;
}
...
{
  auto map = std::make_shared<libernav_rog_map::RogMap>(mapConfig());
  ...
  libernav_scan_planner::SingleScanPlannerConfig config;
  config.optimization_iterations = 5;
  libernav_scan_planner::SingleScanPlanner planner(config, map, footprint());
  libernav_scan_planner::TimedScanTrajectory trajectory;
  std::string message;
...
TEST(SingleScanPlanner, RepairsCollisionOnFinalReferenceEdge)
{
  ...
  libernav_scan_planner::SingleScanPlanner planner(config, map, small_footprint);
  libernav_scan_planner::TimedScanTrajectory trajectory;
  std::string message;
