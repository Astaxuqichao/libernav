...
: nav2_util::LifecycleNode("libernav_costmap_nav", "", [](rclcpp::NodeOptions node_options) {
    return node_options.enable_rosout(false);
  }(options))
{
  RCLCPP_INFO(get_logger(), "[LiberNav] CostmapNavNode created");
}
...
{
  RCLCPP_INFO(get_logger(), "[LiberNav] configuring costmaps and action servers");
  ...
  controller_server_ =
    std::make_shared<libernav_nav::ControllerCostmapServer>(
  ...
  planner_server_ =
    std::make_shared<libernav_nav::PlannerCostmapServer>(
  ...
  behavior_server_ = std::make_shared<libernav_nav::BehaviorCostmapServer>(
...
RCLCPP_COMPONENTS_REGISTER_NODE(libernav_nav::CostmapNavNode)
