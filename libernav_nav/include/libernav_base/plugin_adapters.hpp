...
{
public:
  explicit RogMapPlannerAdapter(libernav_rogmap_core::GlobalPlanner::Ptr plugin)
  : plugin_(std::move(plugin)) {}
  uint32_t makePlan(
    const geometry_msgs::msg::PoseStamped & start,
    const geometry_msgs::msg::PoseStamped & goal,
...
private:
  libernav_rogmap_core::GlobalPlanner::Ptr plugin_;
};
...
{
public:
  explicit RogMapControllerAdapter(libernav_rogmap_core::Controller::Ptr plugin)
  : plugin_(std::move(plugin)) {}
  uint32_t computeVelocityCommands(
    const geometry_msgs::msg::PoseStamped & pose,
    const geometry_msgs::msg::Twist & velocity,
...
private:
  libernav_rogmap_core::Controller::Ptr plugin_;
};
...
{
public:
  explicit RogMapRecoveryAdapter(libernav_rogmap_core::Recovery::Ptr plugin)
  : plugin_(std::move(plugin)) {}
  uint32_t runBehavior(std::string & message) override
  {
    return plugin_->runBehavior(message);
...
private:
  libernav_rogmap_core::Recovery::Ptr plugin_;
};
