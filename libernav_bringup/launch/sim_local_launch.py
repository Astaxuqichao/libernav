...
def generate_launch_description():
    ...
    libernav_bringup_dir = get_package_share_directory('libernav_bringup')
    ...
    return LaunchDescription([
        DeclareLaunchArgument(
            'map_file',
            default_value=os.path.join(libernav_bringup_dir, 'maps', 'map1.yaml'),
            description='Path to the map file'),
        DeclareLaunchArgument(
            'rviz_config_file',
            default_value=os.path.join(
                libernav_bringup_dir, 'rviz', 'nav2_default_view.rviz'),
        ...
        LogInfo(msg='LiberNav local simulation launched.'),
    ])
