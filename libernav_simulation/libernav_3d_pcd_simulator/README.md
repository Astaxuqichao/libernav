# libernav_3d_pcd_simulator

Terrain-constrained 3D kinematic simulator for LiberNav. It deliberately has no
sensor model: the PCD is a static reference surface, while navigation still
uses the real `libernav_rog_map` input path. The robot receives planar body-frame
...
```bash
ros2 launch libernav_3d_pcd_simulator pcd_terrain_simulator.launch.py \
  pcd_file:=/absolute/path/to/terrain.pcd \
  ground_clearance:=0.5
```

The default terrain map is installed by `libernav_bringup` at
...
```bash
python3 ../libernav_bringup/tools/generate_multilevel_terrain_pcd.py
```

The node is intentionally independent of ROGMap. To test the complete stack,
start this package first and start `libernav_bringup` separately; both processes
