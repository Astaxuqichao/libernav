...
/**
 * Execute path planning action (implements LiberNavActionBase::runImpl)
 *
 * **Execution Flow:**
 ...
 *
 * **Implementation Notes:**
 * - Uses utility function libernav_utility::transformPose for thread-safe
 * transformation
  ...
  std::vector<geometry_msgs::msg::PoseStamped> & global_plan)
{
  ...
  for (iter = plan.begin(); iter != plan.end(); ++iter) {
    ...
    tf_success = libernav_utility::transformPose(
