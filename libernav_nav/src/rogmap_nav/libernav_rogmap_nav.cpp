...
{
  RCLCPP_INFO(get_logger(), "[LiberNav] RogMapNavNode created");
}
...
{
  RCLCPP_INFO(get_logger(), "[LiberNav] configuring ROG map and action servers");
  ...
  rog_map_ = std::make_shared<libernav_rog_map::RogMapROS>(
    "rog_map", std::string{get_namespace()}, "rog_map");
  rog_map_->configure();
  tf_listener_ptr_ = rog_map_->getTfBuffer();

  robot_info_ = std::make_shared<libernav_utility::RobotInformation>(
  ...
  controller_server_ =
    std::make_shared<libernav_nav::ControllerRogMapServer>(
  ...
  planner_server_ =
    std::make_shared<libernav_nav::PlannerRogMapServer>(
    rog_map_, robot_info_, std::string{get_namespace()});
  planner_server_->configure();
  planner_server_thread_ =
    std::make_unique<nav2_util::NodeThread>(planner_server_);

  behavior_server_ = std::make_shared<libernav_nav::RecoveryRogMapServer>(
  ...
  RCLCPP_INFO(
    get_logger(),
    "[LiberNav] configured: global_frame=%s robot_frame=%s odom_topic=%s",
...
RogMapNavNode::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  // Keep the existing LiberNav query service names for client compatibility.
  ...
  RCLCPP_INFO(get_logger(), "[LiberNav] active: planner, controller, behavior, costmaps");
  return rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::
         CallbackReturn::SUCCESS;
}
...
std::shared_ptr<libernav_rog_map::RogMapROS>
...
uint8_t RogMapNavNode::occupancyToState(libernav_rog_map::OccupancyState state)
{
  using Res = nav2_msgs::srv::CheckPoint::Response;
  if (state == libernav_rog_map::OccupancyState::OCCUPIED) {
    return Res::LETHAL;
  }
  if (state == libernav_rog_map::OccupancyState::UNKNOWN) {
    return Res::UNKNOWN;
  }
  return Res::FREE;
}
  ...
  std::shared_ptr<nav2_msgs::srv::CheckPoint::Response> response)
{
  ...
  libernav_rog_map::Index3D index;
...
RCLCPP_COMPONENTS_REGISTER_NODE(libernav_nav::RogMapNavNode)
