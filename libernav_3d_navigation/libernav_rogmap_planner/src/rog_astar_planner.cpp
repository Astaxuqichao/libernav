...
#include "libernav_rogmap_planner/rog_astar_planner.hpp"
...
namespace libernav_rogmap_planner
{
...
bool RogAStarPlanner::isStateValid(const geometry_msgs::msg::Point & point) const
{
  ...
  if (unknown_as_obstacle_ && state == libernav_rog_map::OccupancyState::UNKNOWN) {
    return false;
  }
...
void RogAStarPlanner::buildTrajectory(
  ...
  const geometry_msgs::msg::PoseStamped & start,
  const geometry_msgs::msg::PoseStamped & goal,
  libernav_rogmap_core::Trajectory3D & trajectory) const
{
...
PLUGINLIB_EXPORT_CLASS(
  libernav_rogmap_planner::RogAStarPlanner,
